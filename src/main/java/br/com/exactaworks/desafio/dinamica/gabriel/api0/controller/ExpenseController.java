...
public class ExpenseController {
    ...
    public ResponseEntity<?> getExpense(@PathVariable("id") int expenseId, Pageable pageable, PersistentEntityResourceAssembler resourceAssembler) {
        try {
            log.info("ExpenseController::: " + expenseId);

            Expense expense = expenseService.getExpense(expenseId);

            Link selfLink = WebMvcLinkBuilder.linkTo(methodOn(this.getClass()).getExpense(expenseId, pageable, resourceAssembler)).withSelfRel();

            Link allExpensesLink = WebMvcLinkBuilder.linkTo(this.getClass()).slash("/expenses").withRel("all expenses");
            ...
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Expense Not Found", exc);
        }
    }
    ...
    public ResponseEntity<?> createExpense(@RequestBody ExpenseDto expenseDto, Pageable pageable, PersistentEntityResourceAssembler resourceAssembler) {
        ...
        Link selfLink = WebMvcLinkBuilder.linkTo(methodOn(this.getClass()).createExpense(expenseDto, pageable, resourceAssembler)).withSelfRel();
        ...
        return new ResponseEntity<EntityModel<Expense>>(expenseResource, responseHeaders, HttpStatus.CREATED);
    }
}
